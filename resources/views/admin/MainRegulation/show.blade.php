...
@section('content')
    <div class="breadcrumbs">
        <div class="col-sm-8">
            <div class="page-header float-left">
                <div class="page-title">
                    <h1>Legislation: {{ $newRegulation->act->legislation_name }}</h1>
                </div>
            </div>
        </div>
        <div class="col-sm-4">
        ...
        <div class="col-sm-12">
            ...
            <div class="card p-5">
                <form id="form" action="/update_new_regulation/{{ $newRegulation->new_regulation_id }}" method="post"
                    enctype="multipart/form-data" class="form form-horizontal">
                    @csrf
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group form-default">
                                <label class="float-label">Regulation<span class="text-danger">*</span></label>
                                <input type="text" name="new_regulation_title" value="{{ $newRegulation->new_regulation_title }}"
                                    class="form-control mb-3" placeholder="Enter Act Title">
                            </div>
                        </div>
                        <div class="col-md-12">
                            <div class="form-group form-default">
                                <label class="float-label">Regulation NO.<span class="text-danger">*</span></label>
                                <textarea name="new_regulation_no" class="form-control mb-3" placeholder="Enter Act No" id="act_no" cols="30"
                                    rows="3">{{ $newRegulation->new_regulation_no }}</textarea>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group form-default">
                                <label class="float-label">Enactment Date<span class="text-danger">*</span></label>
                                <input type="date" name="enactment_date" value="{{ $newRegulation->enactment_date }}"
                        ...
                        <div class="col-md-6">
                            <div class="form-group form-default">
                                ...
                                <input type="date" name="enforcement_date" value="{{ $newRegulation->enforcement_date }}"
                                    class="form-control mb-3">
                            </div>
                        </div>
                        <div class="col-md-12">
                            <div class="form-group form-default">
                                ...
                                <input type="text" name="new_regulation_date" value="{{ $newRegulation->new_regulation_date }}"
                        ...
                        <div class="col-md-12">
                            <div class="form-group form-default">
                                ...
                                <input type="text" name="ministry" value="{{ $newRegulation->ministry }}"
                        ...
                        <div class="col-md-12">
                            <div class="form-group form-default">
                                ...
                                <textarea name="new_regulation_description" class="form-control mb-3" placeholder="Enter Act Description" id="act_description"
                                    cols="30" rows="3">{{ $newRegulation->new_regulation_description }}</textarea>
                            </div>
                        </div>
                        @if ($newRegulation->new_regulation_footnote_description)
                                <div class="form-group form-default fa fa-arrow-circle-o-right p-0 col-md-12">
                                    ...
                                    <div class="show-footnote" style="">
                                        <div class="footnote-entry">
                                            <textarea type="text" name="new_regulation_footnote_description" id="ck"
                                                class="form-control ckeditor-replace footnote" placeholder="Enter Footnote Description">
                                                {{$newRegulation->new_regulation_footnote_description }}</textarea>
                                        </div>
                            ...
                            <div id="accordion">
                                ...
                                @foreach($paginatedCollection as $item)
                                        <div class="card">
                                            <div class="card-header d-flex">
                                                <div style="width: 90%; text-align: center;">
                                                    <a class="card-link accordion-title" data-toggle="collapse"
                                                        href="#collapse_chapter_{{ $item['regulation_main_id'] }}">
                                                        {!! preg_replace('/[0-9\[\]\.]/', '', $item['regulation_main_title']) !!}
                                                    </a>
                                                </div>
                                                <div
                                                    style="width: 10%; display: flex; justify-content: center; align-items: center;">
                                                    <a href="{{ url('/add_below_new_regulation_maintype', ['new_regulation_id' => $item['new_regulation_id'],'id' => $item['regulation_main_id']]) }}?page={{ $paginatedCollection->currentPage() }}"
                                                        title="Add Next Main Type" class="px-1">
                                                        <i class="bg-success btn-sm fa fa-plus p-1 text-white"></i>
                                                    </a>
                                                    <a href="{{ url('/delete_regulation_maintype/' . $item['regulation_main_id']) }}"
                                            ...
                                            @if (!empty($item['regulationtbl']))
                                                <div id="collapse_chapter_{{ $item['regulation_main_id'] }}" class="collapse"
                                                    data-parent="#accordion">
                                                    <table class="table table-bordered text-center" id="">
                                                        <tbody>
                                                            @foreach ($item['regulationtbl'] as $value)
                                                                <tr>
                                                                    <td>{{$value['regulations_no']}}</td>
                                                                    <td>{!! preg_replace('/[0-9\[\]\.]/', '', $value['regulations_title']) !!}</td>
                                                                    <td>
                                                                        <a href="/edit_regulationTable/{{ $value['regulations_id'] }}?page={{ $paginatedCollection->currentPage() }}" title="Edit"
                                                                            class="px-1">
                                                                            <i class="bg-secondary btn-sm fa fa-edit p-1 text-white"></i>
                                                                        </a>
                                                                        <a href="/view_regulation_sub/{{ $value['regulations_id'] }}?page={{ $paginatedCollection->currentPage() }}" title="View"
                                                                            class="px-1">
                                                                            <i class="bg-primary btn-sm fa fa-eye p-1 text-white"></i>
                                                                        </a>
                                                                        <a href="{{ url('/delete_regulationstbl/' . $value['regulations_id']) }}"
                                                                            title="Delete" class="px-1"
                                                                            onclick="return confirm('Are you sure ?')">
                                                                            <i class="bg-danger btn-sm fa fa-trash p-1 text-white"></i>
                                                                        </a>
                                                                        <a href="{{ url('/add_below_new_rgtlntbl', ['regulation_main_id' => $value['regulation_main_id'], 'regulations_id' => $value['regulations_id']]) }}?page={{ $paginatedCollection->currentPage() }}"
                                                                             class="px-1">
                                                                            <i class="bg-success btn-sm fa fa-plus p-1 text-white"></i>
                                                                        </a>
                                                                    </td>
