            ...
            <div class="page-header float-right">
                <div class="page-title">
                    <ol class="breadcrumb text-right">
                        <a href="{{ Route('new_regulation', ['id' => $act_id]) }}"><button class="btn btn-success mr-2">Add Regulation</button></a>
                    ...
                    <div class="card-body">
                        <table class="table table-bordered text-center" id="myTable">
                            ...
                            <tbody>
                                ...
                                @foreach ($new_regulation as $item)
                                    <tr>
                                        <td scope="row">{{ $a++ }}</td>
                                        <td class="text-capitalize">{{ $item['new_regulation_title'] }}</td>
                                        <td class="text-capitalize">{{ $item['updated_at'] }}</td>
                                        <td class="text-capitalize d-flex">
                                            <a href="/edit_new_regulation/{{$item['new_regulation_id']}}?page={{ $currentPage }}&perPage={{ $perPage }}" title="Edit" class="px-1 edit-link">
                                                <i class="bg-secondary btn-sm fa fa-edit p-1 text-white"></i></a>
                                            <a href="/view_new_regulation/{{$item['new_regulation_id']}}?page={{ $currentPage }}&perPage={{ $perPage }}" title="View" class="px-1 view-link">
                                                <i class="bg-primary btn-sm fa fa-eye p-1 text-white"></i></a>
                                            <a href="/delete_new_regulation/{{$item['new_regulation_id']}}?page={{ $currentPage }}&perPage={{ $perPage }}" onclick="return confirm('Are you sure?')" title="Delete" class="px-1 delete-link">
                        ...
                        <!-- Pagination Links -->
                        {{ $new_regulation->appends(['page' => $currentPage, 'perPage' => $perPage, 'search' => request('search')])->links() }}
                    </div>
                </div>
            </div>
        </div>
