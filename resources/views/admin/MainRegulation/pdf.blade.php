...
<head>
    ...
    <title>{{ $combinedItems[0]->new_regulation_title }}</title>
...
<body>
    ...
    <div style="padding: 50px 50px !important;">
        <div style="text-align: center; text-transform: uppercase !important; font-size: 20px !important;">
            {{ $combinedItems[0]->new_regulation_title }}
        </div>
                ...
                @foreach ($newRegulation->regulationMain as $regulationMain)
                    <div style="text-align: center; margin-bottom: 0.5rem;">
                        <div style="text-transform: uppercase !important; font-size: 16px !important; font-weight: bold !important; margin-top: 0.4rem;">
                            {!! preg_replace('/[0-9\[\]\.]/', '', $regulationMain->regulation_main_title) !!}
                        </div>
                    </div>
                        ...
                        <div style="text-align: start; margin-top: 0.4rem;">
                            ...
                            @foreach ($regulationMain->regulationtbl as $regulationtblItem)
                                <table style="width: 100%; font-size: 15px !important;">
                                    <tr>
                                        <td class="section-no">
                                            <p>{{ $regulationtblItem->regulations_no }}</p>
                                        </td>
                                        <td class="section-title">
                                            <p>{!! $regulationtblItem->regulations_title !!}</p>
                                        </td>
                                    </tr>
                                </table>
                            @endforeach
    ...
    <div style=" padding: 50px 50px !important; page-break-before: always;">
        <div style="text-align: center; text-transform: uppercase !important;font-size: 24px !important;">
            {{ $combinedItems[0]->new_regulation_title }}</div>
        <div style="text-align: center; font-size: 15px !important; margin-top: 0.4rem;">{!! $combinedItems[0]->new_regulation_no !!}</div>
        <div style="font-size: 13px !important; text-align: right !important;">[{{ $combinedItems[0]->new_regulation_date }}]</div>
        <p style="font-size: 13px !important;">{!! $combinedItems[0]->new_regulation_description !!}</p>
        ...
        <em class="footnote"
            ...
            {!! $combinedItems[0]->new_regulation_footnote_description !!}</em>
                        ...
                        @foreach ($newRegulation->regulationMain as $regulationMain)
                            <div style="text-align: center">
                                <div style="text-transform: uppercase !important; font-size: 16px !important; margin-top: 0.4rem;">
                                    {!! $regulationMain->regulation_main_title !!}
                                </div>
                            </div>
                            @if (!empty($regulationMain->regulationtbl))  
                                    ...
                                    @foreach ($regulationMain->regulationtbl as $regulationtblItem)
                                        <div style="font-size: 15px !important; margin-top: 25px !important; padding: 0; line-height: 0.9 !important;">
                                            <div style="margin: 0; padding: 0;">
                                                <p style="margin: 0;font-weight: bold;">{{ $regulationtblItem->regulations_no }}</p>
                                            </div>
                                            <div style="margin: 0; padding: 0;">
                                                <p style="margin: 0;font-weight: bold;">{!! $regulationtblItem->regulations_title !!}</p>
                                            </div>
                                        </div>
                                        <div style="padding-left: 18px;">
                                            <span style=" margin-bottom: 5px!important;">{!! $regulationtblItem->regulations_content !!}</span>
                                        </div>
                                        @if (!empty($regulationtblItem->regulationSub))
                                            @foreach ($regulationtblItem->regulationSub as $regulationSubItem)
                                                <div class="section-container" style="margin-top: 0.4rem; font-size: 15px !important; padding-left:25px!important;">
                                                    <div class="section-item sub-section-no ">{{ $regulationSubItem->regulation_sub_no }}</div>
                                                    <div class="section-item sub-section-content" style="margin-top:-12px!important">{!! $regulationSubItem->regulation_sub_content !!}</div>
                                                </div>
                                            @endforeach
                                        @endif
