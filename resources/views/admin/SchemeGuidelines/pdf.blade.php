...
<head>
    ...
    <title>{{ $combinedItems[0]->new_scheme_guidelines_title }}</title>
...
<body>
    ...
    <div style="padding: 50px 50px !important;">
        <div style="text-align: center; text-transform: uppercase !important; font-size: 20px !important;">
            {{ $combinedItems[0]->new_scheme_guidelines_title }}
        </div>
                ...
                @foreach ($newSchemeGuidelines->schemeGuidelinesMain as $schemeGuidelinesMain)
                    <div style="text-align: center; margin-bottom: 0.5rem;">
                        <div style="text-transform: uppercase !important; font-size: 16px !important; font-weight: bold !important; margin-top: 0.4rem;">
                            {!! preg_replace('/[0-9\[\]\.]/', '', $schemeGuidelinesMain->scheme_guidelines_main_title) !!}
                        </div>
                    </div>
                        ...
                        <div style="text-align: start; margin-top: 0.4rem;">
                            ...
                            @foreach ($schemeGuidelinesMain->schemeGuidelinestbl as $schemeGuidelinestblItem)
                                <table style="width: 100%; font-size: 15px !important;">
                                    <tr>
                                        <td class="section-no">
                                            <p>{{ $schemeGuidelinestblItem->scheme_guidelines_no }}</p>
                                        </td>
                                        <td class="section-title">
                                            <p>{!! $schemeGuidelinestblItem->scheme_guidelines_title !!}</p>
                                        </td>
                                    </tr>
                                </table>
                            @endforeach
    ...
    <div style=" padding: 50px 50px !important; page-break-before: always;">
        <div style="text-align: center; text-transform: uppercase !important;font-size: 24px !important;">
            {{ $combinedItems[0]->new_scheme_guidelines_title }}</div>
        <div style="text-align: center; font-size: 15px !important; margin-top: 0.4rem;">{!! $combinedItems[0]->new_scheme_guidelines_no !!}</div>
        <div style="font-size: 13px !important; text-align: right !important;">[{{ $combinedItems[0]->new_scheme_guidelines_date }}]</div>
        <p style="font-size: 13px !important;">{!! $combinedItems[0]->new_scheme_guidelines_description !!}</p>
        ...
        <em class="footnote"
            ...
            {!! $combinedItems[0]->new_scheme_guidelines_footnote_description !!}</em>
                ...
                @foreach ($newSchemeGuidelines->schemeGuidelinesMain as $schemeGuidelinesMain)
                            <div style="text-align: center">
                                <div style="text-transform: uppercase !important; font-size: 16px !important; margin-top: 0.4rem;">
                                    {!! $schemeGuidelinesMain->scheme_guidelines_main_title !!}
                                </div>
                            </div>
                            @if (!empty($schemeGuidelinesMain->schemeGuidelinestbl)) 
                                ...
                                @foreach ($schemeGuidelinesMain->schemeGuidelinestbl as $schemeGuidelinestblItem)
                                        <div style="font-size: 15px !important; margin-top: 25px !important; padding: 0; line-height: 0.9 !important;">
                                            <div style="margin: 0; padding: 0;">
                                                <p style="margin: 0;font-weight: bold;">{{ $schemeGuidelinestblItem->scheme_guidelines_no }}</p>
                                            </div>
                                            <div style="margin: 0; padding: 0;">
                                                <p style="margin: 0;font-weight: bold;">{!! $schemeGuidelinestblItem->scheme_guidelines_title !!}</p>
                                            </div>
                                        </div>
                                        <div style="padding-left: 18px;">
                                            <span style=" margin-bottom: 5px!important;">{!! $schemeGuidelinestblItem->scheme_guidelines_content !!}</span>
                                        </div>
                                        @if (!empty($schemeGuidelinestblItem->schemeGuidelinesSub))
                                            @foreach ($schemeGuidelinestblItem->schemeGuidelinesSub as $schemeGuidelinesSub)
                                                <div class="section-container" style="margin-top: 0.4rem; font-size: 15px !important; padding-left:25px!important;">
                                                    <div class="section-item sub-section-no ">{{ $schemeGuidelinesSub->scheme_guidelines_sub_no }}</div>
                                                    <div class="section-item sub-section-content" style="margin-top:-12px!important">{!! $schemeGuidelinesSub->scheme_guidelines_sub_content !!}</div>
                                                </div>
                                            @endforeach
                                        @endif
