            ...
            <div class="page-header float-right">
                <div class="page-title">
                    <ol class="breadcrumb text-right">
                        <a href="/get_act_section/{{$rule->act_id}}?page={{ $currentPage }}"><button class="btn btn-danger">Back</button></a>
                    </ol>
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <strong class="card-title">Arrangement of Sub-Rules</strong>
                    </div>
                    <div class="card-body">
                        <table class="table table-bordered text-center" id="myTable">
                            ...
                            <tbody>
                                @php
                                    $a = 1;
                                @endphp
                                @foreach ($sub_rule as $item)
                                    <tr>
                                        <td scope="row">@php echo $a++; @endphp</td>
                                        <td class="text-capitalize">{{ $item->sub_rule_no }}</td>
                                        <td class="text-capitalize">{!! $item->sub_rule_content !!}</td>
                                        <td class="text-capitalize">{{ $item->updated_at }}</td>
                                        <td class="text-capitalize d-flex justify-content-center">
                                            <a href="{{ url('/delete_sub_rule/' . $item->sub_rule_id) }}" title="Delete"
