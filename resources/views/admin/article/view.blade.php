        ...
        <div class="col-sm-8">
            <div class="page-header float-left">
                <div class="page-title">
                    <h1>Article : {{ $article->section_title }}</h1>
                </div>
            </div>
        </div>
        <div class="col-sm-4">
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <strong class="card-title">Arrangement of Sub-Articles</strong>
                    </div>
                    <div class="card-body">
                        <table class="table table-bordered text-center" id="myTable">
                            <thead class="thead-light">
                                <tr>
                                    ...
                                    <th scope="col">Sub-Article No.</th>
                            ...
                            <tbody>
                                @php
                                    $a = 1;
                                @endphp
                                @foreach ($sub_article as $item)
                                    <tr>
                                        <td scope="row">@php echo $a++; @endphp</td>
                                        <td class="text-capitalize">{{ $item->sub_article_no }}</td>
                                        <td class="text-capitalize">{!! $item->sub_article_content !!}</td>
                                        <td class="text-capitalize">{{ $item->updated_at }}</td>
                                        <td class="text-capitalize d-flex justify-content-center">
                                            <a href="{{ url('/delete_sub_article/' . $item->sub_article_id) }}" title="Delete"
