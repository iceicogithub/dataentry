...
@section('content')
    <div class="breadcrumbs">
        <div class="col-sm-8">
            <div class="page-header float-left">
                <div class="page-title">
                    <h1>Rule : {{ $newOrdinance->new_ordinance_title }}</h1>
                </div>
            </div>
        </div>
        <div class="col-sm-4">
            <div class="page-header float-right">
                <div class="page-title">
                    <ol class="breadcrumb text-right">
                        <a href="/add_ordinance/{{ $newOrdinance->new_ordinance_id }}" class="mr-2"><button class="btn btn-success">Add
                                Index</button></a>
                        <a href="/get_ordinance/{{ $newOrdinance->act_id }}"><button class="btn btn-danger">Back</button></a>
                    </ol>
        ...
        <div class="col-sm-12">
            ...
            <div class="card p-5">
                <form id="form" action="/update_new_ordinance/{{ $newOrdinance->new_ordinance_id }}" method="post"
                    enctype="multipart/form-data" class="form form-horizontal">
                    @csrf
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group form-default">
                                <label class="float-label">Ordinance<span class="text-danger">*</span></label>
                                <input type="text" name="new_ordinance_title" value="{{ $newOrdinance->new_ordinance_title }}"
                                    class="form-control mb-3" placeholder="Enter Act Title">
                            </div>
                        </div>
                        <div class="col-md-12">
                            <div class="form-group form-default">
                                <label class="float-label">Ordinance NO.<span class="text-danger">*</span></label>
                                <textarea name="new_ordinance_no" class="form-control mb-3" placeholder="Enter Act No" id="act_no" cols="30"
                                    rows="3">{{ $newOrdinance->new_ordinance_no }}</textarea>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group form-default">
                                <label class="float-label">Enactment Date<span class="text-danger">*</span></label>
                                <input type="date" name="enactment_date" value="{{ $newOrdinance->enactment_date }}"
                        ...
                        <div class="col-md-6">
                            <div class="form-group form-default">
                                ...
                                <input type="date" name="enforcement_date" value="{{ $newOrdinance->enforcement_date }}"
                                    class="form-control mb-3">
                            </div>
                        </div>
                        <div class="col-md-12">
                            <div class="form-group form-default">
                                <label class="float-label">Ordinance Date<span class="text-danger">*</span></label>
                                <input type="text" name="new_ordinance_date" value="{{ $newOrdinance->new_ordinance_date }}"
                                    class="form-control mb-3" placeholder="Enter Ordinance Date">
                            </div>
                        </div>
                        <div class="col-md-12">
                            <div class="form-group form-default">
                                ...
                                <textarea name="new_ordinance_description" class="form-control mb-3" placeholder="Enter Act Description" id="act_description"
                                    cols="30" rows="3">{{ $newOrdinance->new_ordinance_description }}</textarea>
                            </div>
                        </div>
                        @if ($newOrdinance->new_ordinance_footnote_description)
                                <div class="form-group form-default fa fa-arrow-circle-o-right p-0 col-md-12">
                                    ...
                                    <div class="show-footnote" style="">
                                        <div class="footnote-entry">
                                            <textarea type="text" name="new_ordinance_footnote_description" id="ck"
                                                class="form-control ckeditor-replace footnote" placeholder="Enter Footnote Description">
                                                {{$newOrdinance->new_ordinance_footnote_description }}</textarea>
                                        </div>
                            ...
                            <div id="accordion">
                                ...
                                @foreach($paginatedCollection as $item)
                                        <div class="card">
                                            <div class="card-header d-flex">
                                                <div style="width: 90%; text-align: center;">
                                                    <a class="card-link accordion-title" data-toggle="collapse"
                                                        href="#collapse_chapter_{{ $item['ordinance_main_id'] }}">
                                                        {!! preg_replace('/[0-9\[\]\.]/', '', $item['ordinance_main_title']) !!}
                                                    </a>
                                                </div>
                                                <div
                                                    style="width: 10%; display: flex; justify-content: center; align-items: center;">
                                                    <a href="{{ url('/add_below_new_ordinance_maintype', ['new_order_id' => $item['new_ordinance_id'],'id' => $item['ordinance_main_id']]) }}?page={{ $paginatedCollection->currentPage() }}"
                                                        title="Add Next Main Type" class="px-1">
                                                        <i class="bg-success btn-sm fa fa-plus p-1 text-white"></i>
                                                    </a>
                                                    <a href="{{ url('/delete_ordinance_maintype/' . $item['ordinance_main_id']) }}"
                                            ...
                                            @if (!empty($item['ordinancetbl']))
                                                <div id="collapse_chapter_{{ $item['ordinance_main_id'] }}" class="collapse"
                                                    data-parent="#accordion">
                                                    <table class="table table-bordered text-center" id="">
                                                        <tbody>
                                                            @foreach ($item['ordinancetbl'] as $value)
                                                                <tr>
                                                                    <td>{{$value['ordinances_no']}}</td>
                                                                    <td>{!! preg_replace('/[0-9\[\]\.]/', '', $value['ordinances_title']) !!}</td>
                                                                    <td>
                                                                        <a href="/edit_ordinanceTable/{{ $value['ordinances_id'] }}?page={{ $paginatedCollection->currentPage() }}" title="Edit"
                                                                            class="px-1">
                                                                            <i class="bg-secondary btn-sm fa fa-edit p-1 text-white"></i>
                                                                        </a>
                                                                        <a href="/view_ordinance_sub/{{ $value['ordinances_id'] }}?page={{ $paginatedCollection->currentPage() }}" title="View"
                                                                            class="px-1">
                                                                            <i class="bg-primary btn-sm fa fa-eye p-1 text-white"></i>
                                                                        </a>
                                                                        <a href="{{ url('/delete_ordinancestbl/' . $value['ordinances_id']) }}"
                                                                            title="Delete" class="px-1"
                                                                            onclick="return confirm('Are you sure ?')">
                                                                            <i class="bg-danger btn-sm fa fa-trash p-1 text-white"></i>
                                                                        </a>
                                                                        <a href="{{ url('/add_below_new_ordinancetbl', ['ordinance_main_id' => $value['ordinance_main_id'], 'ordinances_id' => $value['ordinances_id']]) }}?page={{ $paginatedCollection->currentPage() }}"
                                                                             class="px-1">
                                                                            <i class="bg-success btn-sm fa fa-plus p-1 text-white"></i>
                                                                        </a>
                                                                    </td>
            ...
            $(document).on('click', '.add-multi-footnote', function() {
                ...
                var newSection = `<div class="form-group form-default fa fa-arrow-circle-o-right p-0 col-md-12 footnote-addition">
                                        ...
                                        <div class="show-footnote" style="display: none">
                                            <textarea type="text" name="new_ordinance_footnote_description" class="form-control ckeditor-replace footnote"></textarea>
                                        </div>
