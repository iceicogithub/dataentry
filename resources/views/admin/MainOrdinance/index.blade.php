            ...
            <div class="page-header float-right">
                <div class="page-title">
                    <ol class="breadcrumb text-right">
                        <a href="{{ Route('new_ordiance', ['id' => $act_id]) }}"><button class="btn btn-success mr-2">Add Ordinance</button></a>
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <strong class="card-title">Ordinance Table</strong>
                    </div>
                    <div class="card-body">
                        <table class="table table-bordered text-center" id="myTable">
                            ...
                            <tbody>
                                @php $a=1; @endphp
                                @foreach ($new_ordinance as $item)
                                    <tr>
                                        <td scope="row">@php echo $a++; @endphp</td>
                                        <td class="text-capitalize">{{ $item['new_ordinance_title'] }}</td>
                                        <td class="text-capitalize">{{ $item['updated_at'] }}</td>
                                        <td class="text-capitalize d-flex">
                                            <a href="/edit_new_ordinance/{{$item['new_ordinance_id']}}?page={{ $currentPage }}" title="Edit" class="px-1"><i
                                                    class="bg-secondary btn-sm fa fa-edit p-1 text-white"></i></a>
                                                    <a href="/view_new_ordinance/{{$item['new_ordinance_id']}}?page={{ $currentPage }}" title="View" class="px-1"><i
                                                        class="bg-primary btn-sm fa fa-eye p-1 text-white"></i></a>
                                            <a href="/delete_new_ordinance/{{$item['new_ordinance_id']}}?page={{ $currentPage }}" onclick="return confirm('Are you sure ?')" title="Delete" class="px-1"><i
                         ...
                         {{ $new_ordinance->appends(['page' => $currentPage])->links() }}
                    </div>
                </div>
            </div>
        </div>
