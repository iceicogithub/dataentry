            ...
            <div class="page-header float-right">
                <div class="page-title">
                    <ol class="breadcrumb text-right">
                        <a href="{{ Route('new_policy', ['id' => $act_id]) }}"><button class="btn btn-success mr-2">Add Policy</button></a>
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <strong class="card-title">Policy Table</strong>
                    </div>
                    <div class="card-body">
                            ...
                            <tbody>
                                ...
                                @foreach ($policy as $item)
                                    <tr>
                                        ...
                                        <td class="text-capitalize">{{ $item['policy_title'] }}</td>
                                        <td class="text-capitalize">{{ $item['updated_at'] }}</td>
                                        <td class="text-capitalize d-flex">
                                            <div class="row" style="justify-content: center">
                                                <button type="button" class="btn btn-warning" onclick="setActAmendmentId('{{ $item['policy_id'] }}', '{{ $item['policy_pdf'] }}')" data-bs-toggle="modal" data-bs-target="#exampleModal">
                                                    Upload Pdf
                                                </button>
                                                <a href="/edit_policy/{{$item['policy_id']}}" title="Edit" class="px-1 edit-link"><i
                                                    class="bg-secondary btn-sm fa fa-edit p-1 text-white"></i></a>
                                                <a href="/view_policy/{{$item['policy_id']}}?page={{ $currentPage }}" title="View" class="px-1"><i
                                                    class="bg-primary btn-sm fa fa-eye p-1 text-white view-link"></i></a>
                                                <a href="/delete_policy/{{$item['policy_id']}}?page={{ $currentPage }}" onclick="return confirm('Are you sure ?')" title="Delete" class="px-1"><i
                        ...
                        {{ $policy->appends(['page' => $currentPage])->links() }}
                    </div>
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form id="uploadpolicy" action="" method="post" enctype="multipart/form-data">
                                        @csrf
                                    <input type="hidden" id="policyIdInput" name="policy_id">
                                    ...
                                    <input type="file" name="policy_pdf" class="form-control mb-3"
                                         required>
...
@section('script')
<script>
     function setActAmendmentId(policyId, policyPdf) {
        console.log(policyPdf);
        document.getElementById('policyIdInput').value = policyId;
        document.getElementById('uploadpolicy').action = "/update_policy_pdf/" + policyId;
        var pdfLink = document.getElementById('pdfLink');
        if (policyPdf) {
            console.log('cbnm');
            pdfLink.innerHTML = policyPdf;
            console.log(pdfLink.innerHTML);
            pdfLink.setAttribute('href', '{{ asset("admin/policy") }}/' + policyPdf);
