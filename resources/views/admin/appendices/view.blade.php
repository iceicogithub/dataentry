        ...
        <div class="col-sm-8">
            <div class="page-header float-left">
                <div class="page-title">
                    <h1>Appendices : {{ $appendices->section_title }}</h1>
                </div>
            </div>
        </div>
        <div class="col-sm-4">
            <div class="page-header float-right">
                <div class="page-title">
                    <ol class="breadcrumb text-right">
                        <a href="/get_act_section/{{$appendices->act_id}}?page={{ $currentPage }}"><button class="btn btn-danger">Back</button></a>
                    </ol>
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <strong class="card-title">Arrangement of Sub-Appendices</strong>
                    </div>
                    <div class="card-body">
                        <table class="table table-bordered text-center" id="myTable">
                            <thead class="thead-light">
                                <tr>
                                    ...
                                    <th scope="col">Sub-Appendices No.</th>
                            ...
                            <tbody>
                                @php
                                    $a = 1;
                                @endphp
                                @foreach ($sub_appendices as $item)
                                    <tr>
                                        <td scope="row">@php echo $a++; @endphp</td>
                                        <td class="text-capitalize">{{ $item->sub_appendices_no }}</td>
                                        <td class="text-capitalize">{!! $item->sub_appendices_content !!}</td>
                                        <td class="text-capitalize">{{ $item->updated_at }}</td>
                                        <td class="text-capitalize d-flex justify-content-center">
                                            <a href="{{ url('/delete_sub_appendices/' . $item->sub_appendices_id) }}" title="Delete"
